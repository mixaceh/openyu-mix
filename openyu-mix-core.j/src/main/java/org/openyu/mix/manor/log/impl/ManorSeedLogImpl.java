...
@Table(name = "mix_manor_seed_log")
@SequenceGenerator(name = "mix_manor_seed_log_g", sequenceName = "mix_manor_seed_log_s", allocationSize = 1)
//when use ehcache, config in ehcache.xml
@Cache(usage = CacheConcurrencyStrategy.READ_WRITE, region = "org.openyu.mix.manor.log.impl.ManorSeedLogImpl")
@Proxy(lazy = false)
@org.hibernate.annotations.Table(appliesTo = "mix_manor_seed_log", indexes = { @org.hibernate.annotations.Index(name = "idx_mix_manor_seed_log_1", columnNames = {
		"role_id", "log_date", "culture_type" }) })
//--------------------------------------------------
//search
//--------------------------------------------------
//@Indexed
public class ManorSeedLogImpl extends AppLogEntitySupporter implements ManorSeedLog
{
	...
	@Id
	@Column(name = "seq")
	@GeneratedValue(strategy = GenerationType.AUTO, generator = "mix_manor_seed_log_g")
	public Long getSeq()
	{
		return seq;
	}
