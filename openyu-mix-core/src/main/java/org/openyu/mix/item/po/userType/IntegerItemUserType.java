...
public class IntegerItemUserType extends BaseUserTypeSupporter {
	...
	public <R, T> R marshal(T value, SessionImplementor session) {
		...
		Map<Integer, Item> src = (Map<Integer, Item>) value;
		// vol
	 ...
	 * v1 由物件組成欄位
	 */
	public String assembleBy_1(Map<Integer, Item> src) {
		StringBuilder result = new StringBuilder();
		//
		result.append(src.size());// 數量
		for (Map.Entry<Integer, Item> entry : src.entrySet()) {
	...
	protected Map<Integer, Item> disassembleBy_1(StringBuilder src) {
		Map<Integer, Item> result = new LinkedHashMap<Integer, Item>();
		if (src == null) {
			return result;
		}
