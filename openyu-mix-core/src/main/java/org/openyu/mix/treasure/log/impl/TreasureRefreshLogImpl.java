...
@DynamicInsert
@DynamicUpdate
@Table(name = "treasure_refresh_log", indexes = {
		@Index(name = "treasure_refresh_log_role_id_log_date", columnList = "role_id,log_date") })
@SequenceGenerator(name = "sg_treasure_refresh_log", sequenceName = "seq_treasure_refresh_log", allocationSize = 1)
...
public class TreasureRefreshLogImpl extends AppLogEntitySupporter implements TreasureRefreshLog {
	...
	@Id
	@Column(name = "seq")
	@GeneratedValue(strategy = GenerationType.AUTO, generator = "sg_treasure_refresh_log")
	public Long getSeq() {
		return seq;
	}
